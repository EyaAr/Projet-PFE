...
public class CompanyController {
	...
	@GetMapping("/gettotalCompanys")
	@CrossOrigin(origins = "http://localhost:4200")
	public ResponseEntity<List<Integer>> gettotalCompanys() throws Exception
	{
		List<Company> companys = companyService.getAllCompanys();
		List<Integer> companysCount = new ArrayList<>();
		companysCount.add(companys.size());
		return new ResponseEntity<List<Integer>>(companysCount, HttpStatus.OK);
	}
	
	
	@DeleteMapping("/deleteCompany/{company-id}")
	@CrossOrigin(origins = "http://localhost:4200")
	@ResponseBody
	public void deleteCompany(@PathVariable("company-id") Integer id) {
